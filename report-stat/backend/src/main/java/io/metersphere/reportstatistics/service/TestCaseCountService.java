...
public class TestCaseCountService {
    ...
    @Resource
    private TestCaseRemoteService testCaseRemoteService;
    @Resource
    private ApiCaseRemoteService apiCaseRemoteService;
    @Resource
    private ScenarioRemoteService scenarioRemoteService;
    @Resource
    private PerformanceRemoteService performanceRemoteService;
    @Resource
    private TestCaseTemplateRemoteService testCaseTemplateRemoteService;
    ...
    public TestCaseCountResponse getReport(TestCaseCountRequest request) {
        ...
        //根据X轴（分组计算字段）来整理不同表对应的字段 注意：x轴维护人/查询条件有维护人时 不查接口和性能； x轴为用例优先级/查询条件有用例优先级的，不查性能
        ...
        switch (request.getXaxis()) {
            ...
            case "caselevel":
                ...
                request.setLoadCaseGroupColumn("'无用例优先级'");
                selectLoad = false;
                break;
            default:
        ...
        if (yAxisSelectApi && DiscoveryUtil.hasService(MicroServiceName.API_TEST)) {
            ...
            apiCaseCountResult = apiCaseRemoteService.countTestCaseByRequest(request);
        ...
        if (yAxisSelectLoad && DiscoveryUtil.hasService(MicroServiceName.PERFORMANCE_TEST)) {
            ...
            loadCaseCount = performanceRemoteService.countTestCaseByRequest(request);
    ...
    private Map<String, String> getUserIdMap() {
        List<User> userList = baseUserService.getUserList();
    ...
    private Map<String, String> getCaseStatusMap(String projectId) {
        ...
        TestCaseTemplateDTO testCaseTemplate = testCaseTemplateRemoteService.selectTemplateByProjectId(projectId);
        if (testCaseTemplate != null && CollectionUtils.isNotEmpty(testCaseTemplate.getCustomFields())) {
            for (CustomField customField : testCaseTemplate.getCustomFields()) {
                if (StringUtils.equals(customField.getName(), "用例状态")) {
                    List<Map<String, Object>> optionsArr = JSON.parseArray(customField.getOptions());
                    for (Map<String, Object> jsonObject : optionsArr) {
                        if (jsonObject.containsKey("value") && jsonObject.containsKey("text") &&
                                !StringUtils.equalsAnyIgnoreCase(String.valueOf(jsonObject.get("value")), "Prepare", "Error", "Success", "Trash", "Underway", "Starting", "Saved")) {
                            caseStatusMap.put(String.valueOf(jsonObject.get("value")), String.valueOf(jsonObject.get("text")));
                        }
                    }
                }
            }
                    ...
                    for (Map<String, Object> jsonObject : optionsArr) {
                        if (jsonObject.containsKey("value") && jsonObject.containsKey("text")) {
                            ...
                            if (!StringUtils.equalsAnyIgnoreCase(value, "Prepare", "Error", "Success", "Trash", "Underway", "Starting", "Saved",
                                ...
                                statusMap.put("id", String.valueOf("value"));
                                statusMap.put("label", String.valueOf("text"));
                                caseStatusList.add(statusMap);
                            }
                        }
                    }
                } else if (StringUtils.equals(customField.getName(), "用例优先级")) {
                    List<Map<String, Object>> optionsArr = JSON.parseArray(customField.getOptions());
                    for (Map<String, Object> jsonObject : optionsArr) {
                        if (jsonObject.containsKey("value") && jsonObject.containsKey("text") &&
                                !StringUtils.equalsAnyIgnoreCase(String.valueOf("value"), "P0", "P1", "P2", "P3")) {
                            Map<String, String> levelMap = new HashMap<>();
                            levelMap.put("id", String.valueOf("value"));
                            levelMap.put("label", String.valueOf("text"));
