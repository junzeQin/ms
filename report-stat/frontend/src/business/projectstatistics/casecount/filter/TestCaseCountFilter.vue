    ...
    <el-card class="ms-card">
      ...
      <el-row class="ms-row">
        ...
        <div v-if="option.timeType === 'dynamicTime'" style="width: 70%;margin-left: 20px;float: left">
          ...
          <el-select class="ms-http-select" size="small" v-model="option.timeFilter.timeRange"
                ...
                <el-row v-for="filterItem in option.filters" :key="filterItem.id" style="margin-bottom: 5px">
                  <el-col :span="24" name="itemOptions">
                    <el-select style="width: 100px" class="ms-http-select" size="small" v-model="filterItem.type">
                      <el-option v-for="item in getFilterOptionKey(filterItem.type)" :key="item.type" :label="item.name"
                                 :value="item.type"/>
                    </el-select>
                    <span style="margin-left:10px;margin-right:10px">{{
                        $t('commons.report_statistics.report_filter.belone')
                      }}</span>

                    <el-select style="width:173px" class="ms-http-select" size="small" multiple filterable
                               v-model="filterItem.values"
                               v-if="getFilterOptions(filterItem.type).length > 0">
                      <el-option v-for="itemOption in getFilterOptions(filterItem.type)" :key="itemOption.id"
      ...
      <el-row align="middle">
        <el-button style="margin-left: 200px;margin-top: 20px" type="primary" size="mini" @click="init">
          {{ $t('commons.confirm') }}
        </el-button>
      </el-row>
    </el-card>
...
import {initCountData} from "@/api/report";
...
export default {
  ...
  methods: {
    ...
    initMoreOptionsSelectorValues() {
      ...
      initCountData(selectParam).then(response => {
