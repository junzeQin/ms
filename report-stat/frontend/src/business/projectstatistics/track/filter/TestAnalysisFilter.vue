  ...
  <div v-loading="loading">
    <el-card :style="{height: h + 'px'}" class="ms-card">
      ...
      <el-row class="ms-row">
        ...
        <ms-select-tree size="small" :data="items" :default-key="projectDefaultKey" @getValue="setProjects" :obj="obj"
                        clearable checkStrictly
                        multiple ref="projectSelector"/>
      </el-row>
      <el-row class="ms-row">
        <p>{{ $t('test_track.module.module') }}</p>
        <ms-select-tree size="small" :data="modules" :default-key="moduleDefaultKey" :disabled="disabled"
                        @getValue="setModules" :obj="moduleObj"
                        clearable checkStrictly multiple ref="moduleSelector"/>
      </el-row>
      <el-row class="ms-row">
      ...
      <el-row class="ms-row">
        ...
        <ms-select-tree size="small" :data="maintainerOptions" :default-key="userDefaultKey" @getValue="setUsers"
                        :obj="moduleObj" clearable
                        checkStrictly multiple ref="userSelector"/>
      </el-row>
...
export default {
  ...
  watch: {
    option: {
      handler: function () {
        if (this.syncReport) {
          console.info("filter:" + JSON.stringify(this.option));
  ...
  methods: {
    ...
    initModule() {
      this.result = selectTestCaseNodeList(this.option.projects[0]).then(response => {
        this.modules = response.data;
        this.$refs.moduleSelector.setKeys(this.moduleDefaultKey);
      });
    },
    initUsers() {
      selectUserProjectMember().then(response => {
