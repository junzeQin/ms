<template>
  ...
  <el-dialog :close-on-click-modal="false" :title="$t('operating_log.change_history')" :visible.sync="infoVisible" width="900px"
          ...
          <div v-if="scope.row.details && scope.row.details.columns">
            <div v-for="detail in scope.row.details.columns" :key="detail.id">
              <div v-if="linkDatas.indexOf(detail.columnName)!== -1">
                <el-link style="color: #315efb" @click="openDetail(scope.row,detail)">{{ $t('operating_log.info') }}</el-link>
              </div>
      ...
      <el-table-column prop="newValue" :label="$t('operating_log.after_change')">
        <template v-slot:default="scope">
          <div v-if="scope.row.details && scope.row.details.columns">
            <div v-for="detail in scope.row.details.columns" :key="detail.id">
              <div v-if="linkDatas.indexOf(detail.columnName)!== -1">
                <el-link style="color: #315efb" @click="openDetail(scope.row,detail)">{{
