    ...
    <el-tabs v-model="activeName" @tab-click="handleClick" style="margin-left: 40px">
      <el-tab-pane :label="$t('variables.config')" name="config">
        ...
        <el-row style="margin-top: 10px">
          ...
          <el-col :span="19">
            <el-select v-model="localData.quotedData" size="small" :disabled="disabled">
...
export default {
  ...
  watch: {
    ...
    'localData.quotedData'(newValue) {
      this.$emit('update-quoted-data', newValue);
    }
  },
  mounted() {
    this.localData = { ...this.editData };
  },
  methods: {
    complete(results) {
