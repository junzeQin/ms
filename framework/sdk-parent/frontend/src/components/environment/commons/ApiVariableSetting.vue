<template>
  <ms-edit-dialog
    ...
    <ms-edit-csv v-if="editData.type=='CSV'" ref="csv" :editData.sync="editData" @update-quoted-data="handleQuotedDataUpdate"/>
...
export default {
  ...
  methods: {
    handleQuotedDataUpdate(newValue) {
      this.editData.quotedData = newValue;
    },
    open(item) {
      this.visible = true;
      this.editData = item;
