<template>
  <div>
  <div style="display: flex;margin-top: 48vh;justify-content: center; font-size: 18px;">跳转中...</div>
  <!-- <div class="container">
    <el-row type="flex">
      <el-col :span="12">
        <div class="content">
...
<script>
import {getCurrentUserId, publicKeyEncrypt, setCurrentProjectID} from "../../utils/token";
import {DEFAULT_LANGUAGE, PRIMARY_COLOR, TokenKey} from "../../utils/constants";
...
export default {
  ...
  beforeCreate() {
    ...
    const isEmpty = (obj) => {
      ...
      if (typeof obj === 'string' && (obj.trim().length === 0 || obj === "undefined" || obj === "null")) {
        return true;
      }
    ...
    const getUserSession = () => {
      ...
      for (const item of items) {
        ...
        if (element[0] === 'userSessionId') {
          userSession.sessionId = element[1]
          continue
        }
        if (element[0] === 'csrfToken') {
          userSession.csrfToken = decodeURIComponent(element[1])
          continue
        }
        if (element[0] === 'projectId') {
          userSession.projectId = decodeURIComponent(element[1])
          continue
        }
      }
    ...
    if (!isEmpty(userSession)) {
      ...
      setCurrentProjectID(userSession.projectId)
    }
