...
public class LoginController {
    ...
    @PostMapping(value = "/testTrainLogin")
    @MsAuditLog(module = OperLogModule.AUTH_TITLE, type = OperLogConstants.LOGIN, title = "登录")
    public Mono<ResultHolder> testTrainLogin(@RequestBody LoginRequest request, WebSession session, Locale locale) {
        return Mono.defer(() -> userLoginService.login(request, session, locale).map(new Function<SessionUser, Mono<SessionUser>>() {
                @Override
                public Mono<SessionUser> apply(SessionUser sessionUser) {
                    sessionUser.setUserGroups(null);
                    sessionUser.setGroups(null);
                    sessionUser.setGroupPermissions(null);
