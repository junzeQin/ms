...
public class UserLoginService {
    ...
    public boolean checkUserPassword(String userId, String password) {
        ...
        var users = userMapper.selectByExample(example);
        for (User user : users) {
            if (CodingUtil.matchesPassword(password, user.getPassword())) {
                return true;
            }
        }
    ...
    public boolean checkWhetherChangePasswordOrNot(LoginRequest request) {
        ...
        if (StringUtils.equals("admin", request.getUsername())) {
            ...
            var users = userMapper.selectByExample(example);
            for (User user : users) {
                if (CodingUtil.matchesPassword("metersphere", user.getPassword())) {
                    return true;
                }
            }
