  ...
  <div>
    <el-card class="table-card" v-loading="loading">
      ...
      <template v-slot:header>
        <ms-table-header :create-tip="btnTips" :tip="$t('system.search_by_environment_name')"
...
import EnvGroupCascader from "./EnvGroupCascader";
...
export default {
  ...
  methods: {
    ...
    list() {
      if (!this.projectList || this.projectList.length === 0) {   //没有项目数据的话请求项目数据
        listAllProject().then(res => {
          this.projectList = res.data;  //获取当前工作空间所拥有的项目,
          this.projectFilters = [];
    ...
    getEnvironments(projectIds) {
      ...
      this.loading = getEnvironmentPages(this.currentPage, this.pageSize, this.condition).then(res => {
        let {listObject, itemCount} = res.data;
        this.environments = listObject;
