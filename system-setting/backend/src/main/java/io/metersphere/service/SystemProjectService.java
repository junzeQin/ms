...
public class SystemProjectService {
    ...
    public Project addProject(AddProjectRequest project) {
        ...
        String userId = project.getCreateUser();
        try {
            if (StrUtil.isBlank(userId)) {
                userId = SessionUtils.getUserId();
            }
        } catch (Exception ignored) {
            userId = UserConstants.ADMIN;
        }
        // 创建项目为当前用户添加用户组
        ...
        baseUserMapper.updateLastProjectIdIfNull(project.getId(), userId);
    ...
    public Project createProject(AddProjectRequest projectRequest) {
        Project project = addProject(projectRequest);
        try {
            microService.getForData(MicroServiceName.API_TEST, "/api/definition/mock-environment/" + project.getId());
        } catch (Exception e) {
...
//        }
        ...
        if (StrUtil.isBlank(pjId)) {
            pjId = UUID.randomUUID().toString();
            project.setId(pjId);
        }
        // 创建项目为当前用户添加用户组
        String userId = project.getCreateUser();
        try {
            ...
            project.setCreateUser(UserConstants.ADMIN);
        }
    ...
    public void addProjectVersion(Project project) {
        ...
        projectVersion.setCreateUser(userId);
        projectVersion.setCreateTime(System.currentTimeMillis());
    ...
    private void checkProjectExist(Project project) {
        if (project.getName() != null) {
            ...
            criteria.andNameEqualTo(project.getName()).andIdNotEqualTo(project.getId());
            try {
                criteria.andWorkspaceIdEqualTo(SessionUtils.getCurrentWorkspaceId());
            } catch (Exception ignored) {
            }
