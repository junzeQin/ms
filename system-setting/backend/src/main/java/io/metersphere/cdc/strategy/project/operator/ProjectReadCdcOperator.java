...
@Component()
@Slf4j
public class ProjectReadCdcOperator extends AbstractProjectCdcOperator {
    ...
    public void handle(SinkUpgradeElement<TestTrainProject> sinkElement) {
        ...
        if (!super.validateProject(testTrainProject)) {
            log.warn("ProjectReadCdcOperator#handle validate project do not meet requirements: {}", testTrainProject);
            return;
        }
