...
@Component()
@Slf4j
public class LessonTeamUserDeleteCdcOperator extends AbstractLessonTeamUserCdcOperator {
    ...
    public void handle(SinkUpgradeElement<TestTrainLessonTeamUser> sinkElement) {
        var lessonTeamUser = sinkElement.getBefore();
        if (!super.validateLessonTeamUser(lessonTeamUser)) {
            log.warn("LessonTeamUserDeleteCdcOperator#handle validate lesson team user do not meet requirements: {}", lessonTeamUser);
            return;
        }
        var projectId = super.acquireProjectId(lessonTeamUser);
        var userName = lessonTeamUser.getUserName();
        super.userService.deleteProjectMember(projectId, userName);
    }
