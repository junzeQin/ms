...
@Component()
@Slf4j
public class UserUpdateCdcOperator extends AbstractUserCdcOperator {
    ...
    public void handle(SinkUpgradeElement<TestTrainUser> sinkElement) {
        ...
        if (!super.validateUser(testTrainUser)) {
            log.warn("UserUpdateCdcOperator#handle validate user do not meet requirements: {}", testTrainUser);
            return;
        }
