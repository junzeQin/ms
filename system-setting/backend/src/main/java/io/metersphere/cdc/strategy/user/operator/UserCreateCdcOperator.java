...
@Component()
@Slf4j
public class UserCreateCdcOperator extends AbstractUserCdcOperator {
    ...
    public void handle(SinkUpgradeElement<TestTrainUser> sinkElement) {
        ...
        if (!super.validateUser(testTrainUser)) {
            log.warn("UserCreateCdcOperator#handle validate user do not meet requirements: {}", testTrainUser);
            return;
        }
