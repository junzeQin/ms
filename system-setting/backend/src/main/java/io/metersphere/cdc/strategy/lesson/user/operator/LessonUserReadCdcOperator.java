...
@Component()
@Slf4j
public class LessonUserReadCdcOperator extends AbstractLessonUserCdcOperator {
    ...
    public void handle(SinkUpgradeElement<TestTrainLessonUser> sinkElement) {
        ...
        if (!super.validateLessonUser(lessonUser)) {
            log.warn("LessonUserReadCdcOperator#handle validate lesson user do not meet requirements: {}", lessonUser);
            return;
        }
        super.syncProjectMember(OperatorTypeEnum.r, lessonUser);
    }
