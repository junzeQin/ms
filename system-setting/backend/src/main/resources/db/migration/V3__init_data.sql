...
VALUES ('admin', 'Administrator', 'admin@metersphere.io', '$2a$10$fL0k312zYRPyE6.hXAUC9.7joy48f6ZKjHhxouXSqT9jFzyJ8nUfK', '1', unix_timestamp() * 1000, unix_timestamp() * 1000, NULL, '', NULL,
        NULL);
...
VALUES (-1, '默认组织', '系统默认创建的组织', unix_timestamp() * 1000, unix_timestamp() * 1000);
INSERT INTO workspace (id, organization_id, name, description, create_time, update_time)
VALUES (-2, (SELECT id FROM organization WHERE name LIKE '默认组织'), '默认工作空间', '系统默认创建的工作空间', unix_timestamp() * 1000,
        unix_timestamp() * 1000);
INSERT INTO project (id, workspace_id, name, description, create_time, update_time)
VALUES (-3, (SELECT id FROM workspace WHERE name LIKE '默认工作空间'), '默认项目', '系统默认创建的项目', unix_timestamp() * 1000,
        unix_timestamp() * 1000);
