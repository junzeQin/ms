import {fileDownloadGet, fileDownloadPost, get, post, request, previewFileGet} from 'metersphere-frontend/src/plugins/request'

export function getFileMeta(projectId, userId) {
  return get(`/requirement/count/${projectId}/${userId}`);
}

export function modifyFileMeta(param) {
  return post('/requirement/update', param);
}

export function getFileBytes(id) {
  return get('/requirement/info/' + id);
}
...
export function previewFile(id, name) {
  previewFileGet("/requirement/preview/" + id, name);
}

export function downloadMetaData(id, name) {
  fileDownloadGet("/requirement/download/" + id, name);
}

export function downloadFileZip(request) {
  fileDownloadPost("/requirement/download/zip", request, "文件集.zip");
}

export function batchDeleteMetaData(ids) {
  return post('/requirement/delete/batch', ids);
}

export function deleteFileMetaById(metaId) {
  return get(`/requirement/delete/${metaId}`);
}

export function getAllTypeFileMeta() {
  return get('/requirement/get/type/all');
}

export function getRequirementPages(projectId, goPage, pageSize, condition) {
  return post(`/requirement/project/${projectId}/${goPage}/${pageSize}`, condition);
}

export function moveFileMeta(request) {
  return post('/requirement/move', request);
}
