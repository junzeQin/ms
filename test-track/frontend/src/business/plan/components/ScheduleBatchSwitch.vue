...
import {updateBatchScheduleEnable} from "@/api/remote/plan/test-plan";
...
export default {
  ...
  methods: {
    ...
    saveTest() {
      updateBatchScheduleEnable(this.schedule).then(() => {
        this.$message.success(this.$t('commons.modify_success'));
        this.close();
        this.$emit("refresh");
      });
    },
