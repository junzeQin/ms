    ...
    <ms-table
      ...
      <span v-for="(item, index) in fields" :key="index">
        ...
        <ms-table-column :label="$t('test_track.case.case_desc')" prop="desc" :field="item" min-width="100px">
          <template v-slot:default="scope">
            <el-link @click.stop="getCase(scope.row.id)" style="color:#315efb;">{{ $t('commons.preview') }}</el-link>
          </template>
        </ms-table-column>
...
export default {
  ...
  methods: {
    ...
    exportTestCase(exportType, fieldParam) {
      ...
      fileDownloadPost(url, param, "Neuedu_case_" + this.projectName + fileNameSuffix)
        .then(() => {
          this.loading = false;
