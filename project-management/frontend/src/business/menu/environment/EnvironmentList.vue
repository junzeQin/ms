        ...
        <el-card class="table-card" v-loading="loading">
          ...
          >
            ...
            <el-table-column :label="$t('api_test.environment.socket')" show-overflow-tooltip>
              <template v-slot="scope">
                <a v-if="parseDomainName(scope.row)!='SHOW_INFO'" :href="parseDomainName(scope.row)" target="_blank" style="color: blue">{{ parseDomainName(scope.row) }}</a>
          ...
          <el-table :data="conditions">
            <el-table-column prop="socket" :label="$t('load_test.domain')" show-overflow-tooltip width="180">
              <template v-slot:default="{row}">
                <span v-if='row.conditionType'>{{row.server}}</span>
                <a v-else :href="getUrl(row)" target="_blank" style="color: blue">{{ getUrl(row) }}</a>
...
export default {
  ...
  methods: {
    ...
    getEnvironments(projectIds) {
      ...
      this.loading = getEnvironmentPages(this.currentPage, this.pageSize, this.condition)
        .then(response => {
