<template>
  ...
  >
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template v-slot:default="scope">
            <span v-if="scope.row.type !== GROUP_TYPE.PROJECT">
        ...
        <el-table-column
          ...
          <template v-slot:default="scope">
            <group-permission
              :permissions="scope.row.permissions"
              :selected.sync="tableData"
              :read-only="readOnly"
              :group="group"/>
          </template>
        </el-table-column>
    ...
    <template v-slot:footer>
      ...
      <el-button
        type="primary" @click="onSubmit" size="medium"
        style="margin-top: 10px;margin-left: 5px;" :disabled="readOnly">
        {{ $t('commons.confirm') }}
      </el-button>
    </template>
  </el-dialog>
...
export default {
  ...
  methods: {
    ...
    getGroupJson() {
      this.loading = getUserGroupPermission(this.group).then(res => {
        let data = res.data;
        if (data) {
          data.permissions.splice(4,3);
          data.permissions.splice(7,2);
    ...
    onSubmit() {
      ...
      this.loading = modifyUserGroupPermission(param).then(() => {
