        ...
        <el-form :model="form" label-position="right" label-width="80px" size="small" :rules="rules" v-if="isFormAlive"
          ...
          <el-row style="margin-right: 10px;" type="flex" :gutter="5">
              ...
              <el-form-item>
                <template v-slot>
                  <div style="position: relative;" :class="{'button-color': apiReviewTestScript}">
                    ...
                    <el-button type="primary" size="mini" style="width: 70px;position: absolute; right: 0;top:10px;"
                               @click="handleTest"
            ...
            <el-col :span="menuSpan" class="script-index" v-if="showMenu">
              <ms-dropdown :default-command="form.type" :commands="languages"
                           style="margin-bottom: 5px;margin-left: 15px;margin-top: 30px;"
...
import ApiFuncRelevance from "metersphere-frontend/src/components/environment/snippet/ApiFuncRelevance";
import ScriptNavMenu from "metersphere-frontend/src/components/environment/snippet/ScriptNavMenu";
...
export default {
  ...
  methods: {
    ...
    initFunc(id) {
      this.loading = getCodeSnippetById(id).then(res => {
        this.form = res.data;
        if (!this.form.tags) {
          this.form.tags = [];
        } else {
    ...
    submit() {
      ...
      this.$refs['form'].validate(valid => {
        ...
        this.form.id ? this.update(param) : this.create(param);
      })
    },
    create(obj) {
      this.loading = saveCodeSnippet(obj).then(res => {
        this.$emit("refresh");
        if (res.data) {
    ...
    handleTest() {
      ...
      jSR223Processor.projectId = getCurrentProjectID();
      jSR223Processor.clazzName = TYPE_TO_C.get("JSR223Processor");
      this.runData = jSR223Processor;
